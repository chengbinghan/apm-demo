...
import apm.collector.service.MysqlApiMetaDataService;
import apm.collector.util.CollectorConstant;
import com.navercorp.pinpoint.collector.dao.ApiMetaDataDao;
import com.navercorp.pinpoint.collector.db.DBApiMetaDataDao;
...
public class HbaseApiMetaDataDao implements ApiMetaDataDao {
    ...
    //O===||=================>add by hcb
    @Resource
    private MysqlApiMetaDataService mysqlApiMetaDataService;
    ...
    public void insert(TApiMetaData apiMetaData) {
        ...
        if (true) {
            try {
                mysqlApiMetaDataService.addMysqlApiMetaDataBo(apiMetaData);
            } catch (Exception e) {
                logger.error("insert apiMetaData to mysql failed.");
                e.printStackTrace();
            }
        }
        //Insert record into mysql DB-cody by lmy
        // DBApiMetaDataDao dbApiMetaDataDao = new DBApiMetaDataDao();
        //dbApiMetaDataDao.insertApiMetaDataToDB(apiMetaData);
        //O===||=================>add by hcb end
